...
if __name__ == "__main__":
    # 文件路径
    excel_file = './sift_descriptor_good.xlsx'
    excel_file2 = './sift_descriptor_bad.xlsx'
    excel_file3 = './sift_descriptor_good_(test_set).xlsx'
    output_file = './classification_reports4.xlsx'
    ...
    for category, normal_features in features.items():
        if len(normal_features) > 1:  # 确保有足够的数据进行训练和测试
            ...
            damaged_features = get_feature_from_excel(excel_file2, feature_size=2560, category=category)
            normal_features1 = get_feature_from_excel(excel_file3, feature_size=2560, category=category)
